...
function switchTradeTab(element, tabName) {
    // ၁။ Button Styles ပြောင်းခြင်း (element ရှိမှသာ လုပ်ဆောင်ရန်)
    if (element) {
    ...
    // ၂။ Tab Content အားလုံးကို အရင်ပိတ်ပြီးမှ သက်ဆိုင်ရာတစ်ခုကိုပဲ ဖွင့်ခြင်း
    const tabs = ['positions-tab-content', 'open-orders-tab-content', 'order-history-tab-content'];
    ...
    // ရွေးချယ်လိုက်တဲ့ tab ကို ဖွင့်ခြင်း
...
async function addBuyLongTrade() {
    ...
    try {
        const historySnapshot = await firebase.database().ref(`future_trade_History/${currentUser.uid}`).once('value');
    ...
    try {
        ...
        await firebase.database().ref(`future_trade_History/${currentUser.uid}`).push(tradeData);
...
function renderStoredHistory() {
    ...
    const historyRef = firebase.database().ref(`future_trade_History/${currentUser.uid}`);
                ...
                <div class="trade-card p-4 mb-3" style="background: #1e2329; border-radius: 8px; border-bottom: 1px solid #2b3139;">
                    ...
                    <div class="mt-auto grid grid-cols-3 gap-1">
                        <div class="flex flex-col gap-y-1">
                            ...
                            <div>
                                <span class="text-gray-500 block text-[13px]">Entry Price</span>
                        ...
                        <div class="mt-auto flex flex-col gap-y-1 px-2">
                            ...
                            <div>
                                <span class="text-gray-500 block text-[13px]">Position Size</span>
                            ...
                            <div>
                                <span class="text-gray-500 block text-[13px]">Market Price</span>
                        ...
                        <div class="mt-auto flex flex-col gap-y-1 pl-1 text-right">
                            <div>
                                <span class="text-gray-500 block text-[13px]">Unrealized PNL</span>
                            ...
                            <div class="mt-2">
                                <span class="text-gray-500 block text-[13px]">Liq.Price</span>
...
async function closePosition(tradeId, trade) {
    ...
    document.getElementById('m-symbol').innerText = `${trade.coinSymbol || 'BTC'}/USDT ${trade.marginType || 'Cross'} ${trade.leverage}`;
...
async function executeClosePosition(tradeId, trade, marketPrice, netPnl, margin) {
    try {
        ...
        await firebase.database().ref(`future_trade_History/${currentUser.uid}/${tradeId}`).set(updatedTradeData);
...
async function confirmCheckoutTrade() {
    try {
        ...
        if (typeof addBuyLongTrade === "function") {
            ...
            // ၃။ အောင်မြင်မှ Notification ပြမယ်
...
function renderOrderHistory() {
    ...
    firebase.database().ref(`future_trade_History/${currentUser.uid}`).once('value', (snapshot) => {
        const data = snapshot.val();
                ...
                <div class="trade-card p-4 mb-3" style="background: #1e2329; border-radius: 8px; border-bottom: 1px solid #2b3139;">
                    ...
                    <div class="mt-auto grid grid-cols-3 gap-1">
                        ...
                        <div class="mt-auto flex flex-col gap-y-1 px-2">
                            ...
                            <div>
                                <span class="text-gray-500 block text-[13px]">Close Price</span>
